...
#define LOWER_ZERO_BITS (log_base2(sizeof(md_inst_t)))
#define RPT_INDEX_BITS(cp) (log_base2((cp)->rpt_table_size))
#define RPT_INDEX_MASK(cp) (((int)(pow(2, RPT_INDEX_BITS(cp))) - 1) << LOWER_ZERO_BITS)
#define RPT_INDEX(pc, cp) ((pc & RPT_INDEX_MASK(cp)) >> LOWER_ZERO_BITS)
#define RPT_TAG(pc, cp) (pc >> (RPT_INDEX_BITS(cp) + LOWER_ZERO_BITS))
       ...
       int prefetch_type)   /* prefetcher type */
{
  ...
  /* ECE552 Assignment 4 - BEGIN CODE */
  cp->rpt_table_size = prefetch_type;
  cp->missq_head = 0;
  cp->rpt_table = NULL;
  cp->missq = NULL;
  /* ECE552 Assignment 4 - END CODE */
  ...
  if (cp->prefetch_type > 2) {
    cp->rpt_table = initialize_rpt_table(cp->prefetch_type);
    assert(cp->rpt_table);
    cp->rpt_table_size = cp->prefetch_type;
  }
  else if (cp->prefetch_type == 2) {
    cp->rpt_table = initialize_rpt_table(MAX_RPT_SIZE);
    assert(cp->rpt_table);
    cp->missq = initialize_missq();
    assert(cp->missq);
    cp->rpt_table_size = MAX_RPT_SIZE;
  }
  /* ECE552 Assignment 4 - END CODE */
...
/* ECE552 Assignment 4 - BEGIN CODE*/
// Initialize all tags to 0 and all entries to INIT stage
rpt_entry *initialize_rpt_table(int size) {
  rpt_entry *rpt_table = (rpt_entry *)malloc(size * sizeof(rpt_entry));
  assert(rpt_table);
  int i;
  for (i = 0; i < size; ++i) {
    rpt_table[i].pc_tag = 0;
...
md_addr_t *initialize_missq() {
  md_addr_t *missq = (md_addr_t *)malloc(MAX_MISS_QSIZE * sizeof(md_addr_t));
  assert(missq);
  int i;
  for (i = 0; i < MAX_MISS_QSIZE; ++i) {
    missq[i] = 0;
  }
  return missq;  
}

// Updates the rpt table for the given address
// Use PC to get the tag and upadte the table entry's stride, previous address and state
// Return false if tag is not found in table
bool stride_rpt_update(struct cache_t *cp, md_addr_t addr) {
  md_addr_t pc = get_PC();
  int idx = RPT_INDEX(pc, cp);
  md_addr_t tag = RPT_TAG(pc, cp);
  assert(cp->rpt_table);
  rpt_entry *entry = &(cp->rpt_table[idx]);
...
// Find the given address in the miss queue. If found, return the next address in the buffer
// If not found, return 0
md_addr_t get_next_missq_address (struct cache_t *cp, md_addr_t addr) {
  int i;
  for (i = 0; i < MAX_MISS_QSIZE; ++i) {
    if (cp->missq[i] == addr) {
      // Return the next address in the miss queue
      int missq_idx = (i+1)%MAX_MISS_QSIZE;
      return cp->missq[missq_idx];
    }
  }
  return 0;
}

// Inserts the given address at the head index of the miss queue
void insert_missq_address (struct cache_t *cp, md_addr_t addr) {
  cp->missq[cp->missq_head] = addr;
  cp->missq_head = (cp->missq_head + 1)%MAX_MISS_QSIZE;
}

/* Next Line Prefetcher */
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  md_addr_t addr_next = addr + cp->bsize;
  md_addr_t tagset_next = CACHE_TAGSET(cp, addr_next);
  
  if (!cache_probe(cp, tagset_next)){
    cache_access(cp, Read, tagset_next, NULL, cp->bsize, NULL, NULL, NULL, 1);
  }
}

/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
  bool found = stride_rpt_update(cp, addr);

  md_addr_t pc = get_PC();
  int idx = RPT_INDEX(pc, cp);
  assert(cp->rpt_table);
  rpt_entry *entry = &(cp->rpt_table[idx]);
  md_addr_t addr_next = 0, tagset_next = 0;

  // If the address is not found in the RPT table or if the state is not steady
  // Prefetch the next address in the miss queue, if it exists
  if (!found || entry->state != STEADY ) {
    addr_next = get_next_missq_address(cp, addr);
  }
  if (addr_next == 0) {
    addr_next = addr + entry->stride;
  }

  tagset_next = CACHE_TAGSET(cp, addr_next);
  if (!cache_probe(cp, tagset_next)){
    cache_access(cp, Read, tagset_next, NULL, cp->bsize, NULL, NULL, NULL, 1);
  }
}

void print_rpt_table(rpt_entry *rpt_table, md_addr_t addr, struct cache_t *cp) {
  assert(rpt_table);
  ...
  printf("Idx %d tag 0x%x\n", RPT_INDEX(pc, cp), RPT_TAG(pc, cp));
  for (i = 0; i < cp->rpt_table_size; ++i) {
    entry = &(rpt_table[i]);
    printf("INDEX: %d TAG 0x%x PREV_ADDR 0x%x STRIDE %d STATE %s\n", i, entry->pc_tag, entry->prev_addr, entry->stride, stringFromRPT_States(entry->state));
  }
  printf(".............................\n");
}
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  if (entry->state != NOPRED) {
    ...
    md_addr_t addr_next = addr + entry->stride;
    md_addr_t tagset_next = CACHE_TAGSET(cp, addr_next);
       ...
       int prefetch)    /* 1 if the access is a prefetch, 0 if it is not */
{
  ...
  cp->read_misses++;
    ...
    /* ECE552 Assignment 4 - BEGIN CODE*/
     // Add all the cache-miss addresses at the head index of the miss queue
     if (cp->prefetch_type == 2) {
      insert_missq_address(cp, addr);
     }
     /* ECE552 Assignment 4 - END CODE*/
  }
  else {
